...
Classic bikes are much more used than the electric ones. And in this table we can see the time spent on each type. Docked bikes are the most used, but this is because when they are docked the time still runs until they are parked to a proper station. Rides with classic bikes tend to be longer:
...
Rides with electric bikes are not only a 21% faster, as we can see in the following table, they do an average of an extra 400m compared with clients that stick with classic bikes:
...
Which are the favorite destinations of our clients? In the following map, we indicate the top 5 destination stations:
```{r}
...
For a more visualize experience, we can plot each station in a map, mapping its importance with the size of dots:
```{r}
require(data.table)
...
```
It seems that the Navy Pier of Chicago is the most visited station.

But we can also check the different behaviour between members and casual riders. 
